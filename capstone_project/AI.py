...
def check_and_scan_url(url, api_key):
    ...
    if prediction_label == "good":
        ...
        if scan_result == "bad":
            ...
            print(f"The URL {url} has been marked as bad after URLScan.io analysis.")
        else:
            print(f"The URL {url} is considered safe after URLScan.io analysis.")
    else:
        # If the AI model predicts the URL as unsafe, mark it as bad immediately
        store_bad_link(url, prediction_label)
        print(f"The URL {url} is considered potentially unsafe by the AI model and has been marked as bad.")
